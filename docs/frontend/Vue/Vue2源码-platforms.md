...
Vue.js 是一个跨平台的 MVVM 框架，它可以跑在 web 上，也可以配合 weex 跑在 native 客户端上。platform 是 Vue.js 的入口，2 个目录代表 2 个主要入口，分别打包成运行在 web 上和 weex 上的 Vue.js,这里主要看web上的

### Vue实例挂载的实现

Vue 中我们是通过 `$mount` 实例方法去挂载 `vm` 的，`$mount` 方法在多个文件中都有定义，因为 `$mount` 这个方法的实现是和平台、构建方式都相关，我们重点分析带 `compiler` 版本的 `$mount` 实现。
...
这段代码先缓存了原型上的 `$mount` 方法，再重新定义该方法。

首先，它对 `el` 做了限制，Vue 不能挂载在 `body`、`html` 这样的根节点上。

接下来如果没有定义 `render` 方法，则会把 `el` 或者 `template` 字符串转换成 `render` 方法。
